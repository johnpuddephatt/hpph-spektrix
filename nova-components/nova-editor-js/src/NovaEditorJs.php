...
class NovaEditorJs extends Field
{
    ...
    public function __construct(
        ...
        $this->withMeta([
            "editorSettings" => [
                "placeholder" => config(
                    "nova-editor-js.editorSettings.placeholder",
                    ""
                ),
                "initialBlock" => config(
                    "nova-editor-js.editorSettings.initialBlock",
                    "paragraph"
                ),
                "autofocus" => config(
                    "nova-editor-js.editorSettings.autofocus",
                    false
                ),
            ],
            "toolSettings" => config("nova-editor-js.toolSettings"),
            "uploadImageByFileEndpoint" => route(
                "editor-js-upload-image-by-file"
            ),
            "uploadImageByUrlEndpoint" => route(
                "editor-js-upload-image-by-url"
            ),
        ...
        try {
            ...
            foreach ($blocks as $block) {
                if (isset(static::$htmlRenderCallbacks[$block["type"]])) {
                    $htmlOutput .= static::$htmlRenderCallbacks[$block["type"]](
                        $block
                    );
                }
            }
            ...
            return html_entity_decode(
                view("vendor.nova-editor-js.content", [
                    "content" => $htmlOutput,
                ])->render()
            );
    ...
    {
        if (static::$hasBootedHtmlCallbacks) {
            return;
        }

        static::$htmlRenderCallbacks["test"] = function ($block) {
            return view("nova-editor-js::test", $block["data"])->render();
        };
        ...
        static::$htmlRenderCallbacks["image"] = function ($block) {
            $block["data"]["classes"] = NovaEditorJs::calculateImageClasses(
                $block["data"]
            );
