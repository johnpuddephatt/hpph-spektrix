           <div class="mb-8 bg-yellow">
               <h2 class="type-regular container py-8">
                   {{ $layout->fund_group_title }}
               </h2>
           </div>

           <div class="container my-12 grid gap-4 md:grid-cols-2">

               @foreach ($layout->funds as $fundField)
                   @if ($fundField)
                       ...
                       @if ($fund)
                           <div class="flex h-full flex-col">
                               ...
                               <spektrix-donate x-data="{ amount: {{ $fund->default_donation_amount }} }"
                                   class="mt-auto block border-t border-sand-dark pt-4"
                                   ...
                                   <div class="type-regular mt-2 rounded bg-sand-light p-2" data-success-container
                                       style="display: none;">Donation added. <a class="underline" href="/basket/">Go
                                           to
                                           basket</a></div>
                                   <div data-fail-container style="display: none;">Donation could not be added to
                                       basket
                                   </div>
                               </spektrix-donate>
                           </div>
