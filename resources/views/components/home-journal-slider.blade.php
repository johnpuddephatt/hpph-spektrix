            ...
            <div class="relative z-10 -mt-1 text-white">
                @foreach ($posts as $post)
                    <a href="{{ route('post.show', ['post' => $post->slug]) }}"
                        ...
                        <x-image class="mt-12 block w-full rounded" :src="$post->featuredImage->getUrl('landscape')" :srcset="$post->featuredImage->getSrcset('landscape')" />
                    </a>
                @endforeach
            </div>
        @endif
