    ...
    <div class="container bg-black py-12">
        <a href="{{ route('strand.show', $strand->slug) }}"
            ...
            <div class="min-h-[18rem] bg-black relative my-0 lg:w-1/3 bg-opacity-50 self-stretch">
                ...
                @if ($strand->logo)
                    @icon($strand->logo, ' transition group-hover:delay-[0ms] delay-100 lg:group-hover:opacity-0 lg:group-hover:-translate-y-full absolute z-[5] top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 mx-auto w-72 max-w-full px-8')
                @else
                    <h3
