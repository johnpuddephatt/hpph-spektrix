...
@if ($strand)
    ...
    <a href="{{ route('strand.show', $strand->slug) }}"
        ...
        <div class="{{ $total_slides < 3 ? 'lg:hidden' : ' mb-6' }} type-xs-mono self-start text-white">More from this strand
        </div>
        ...
        <div
            ...
            @if ($strand->logo)
                @icon($strand->logo, ' transition group-hover:delay-[0ms] delay-100 lg:group-hover:opacity-0 lg:group-hover:-translate-y-full absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mx-auto w-72 max-w-full px-12')
            @else
                <h3
        ...
        <div
            ...
            <div class="{{ $total_slides == 2 ? 'px-4 lg:pl-0 lg:pr-2 xl:px-4' : null }} container">
                ...
                <h3
                    ...
                    Part of <span class="!font-bold" style="color: {{ $strand->color }}">{{ $strand->name }}</span>
                </h3>
                <div
                    class="{{ $total_slides == 1 ? 'mb-16 lg:mb-8  type-regular' : 'type-small mb-2' }} {{ $total_slides == 2 ? ' 2xl:type-regular' : '' }} max-w-lg !font-normal">
                    {{ $strand->description }}</div>
                <span class="type-xs-mono group-hover:underline before:inset-0" style="color: {{ $strand->color }}">+
