@if ($settings['alert_enabled'] && $settings['alert_display_until'] > now())
    <a target="_blank" href="{{ $settings['alert_url'] }}"
        class="type-xs-mono fixed left-0 right-0 top-0 z-20 flex bg-yellow py-1 pl-1 text-black shadow lg:fixed lg:left-auto lg:right-16 lg:top-4 lg:w-full lg:max-w-xs lg:rounded-full">
