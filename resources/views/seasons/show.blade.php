...
@section('content')
    ...
    <div id="event-content" class="bg-yellow text-center pt-8 lg:pt-12 lg:pb-16 pb-24">
        ...
        @if ($season->funders_logo)
            <img onload="this.style.width = this.clientWidth/2 + 'px'" src="{{ Storage::url($season->funders_logo) }}"
                alt="" class="px-4 mt-6 mx-auto w-auto h-auto max-w-sm">
        @endif
    </div>

    @if ($instances->count() || $coming_soon_instances->count())
        <div class="bg-black text-yellow">
            <div class="pt-24 pb-16 container">
                ...
                <x-instance-slider :instances="$instances" :coming_soon_instances="$coming_soon_instances" color="#f2d13c" :layout="match (count($instances) + count($coming_soon_instances)) {
                    1 => 'extra-wide',
                    2 => 'wide',
                    default => 'default',
                }"
